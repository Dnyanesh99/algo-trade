...
class HistoricalAggregator:
    ...
    async def aggregate_and_store(self, instrument_id: int, one_min_data: pd.DataFrame) -> list[AggregationResult]:
        ...
        try:
            ...
            for tf in self.timeframes:
                tf_result = await self._process_timeframe_aggregation(instrument_id, df_prepared, tf, start_time)
                results.append(tf_result)

                # TODO: Add record_aggregation_result method to PerformanceMetrics
    ...
    async def _prepare_input_data(self, one_min_data: pd.DataFrame, instrument_id: int) -> Optional[pd.DataFrame]:
        ...
        Args:
            one_min_data: Raw 1-minute data
        ...
        try:
            ...
            if self.validation_enabled:
                # Prepare data for validation (reset index to have timestamp column)
    ...
    async def _process_timeframe_aggregation(
        self, instrument_id: int, df_prepared: pd.DataFrame, timeframe_minutes: int, start_time: datetime
    ) -> AggregationResult:
        """
        ...
        Args:
            instrument_id: Instrument identifier
            df_prepared: Prepared 1-minute data
            timeframe_minutes: Target timeframe in minutes
            start_time: Processing start time for metrics

        Returns:
            AggregationResult with processing outcome
        """
        tf_start_time = datetime.now()
        timeframe_str = f"{timeframe_minutes}min"

        try:
            logger.info(f"Aggregating to {timeframe_str} timeframe for instrument {instrument_id}")

            # Perform resampling aggregation
            aggregated_df = (
                df_prepared.resample(f"{timeframe_minutes}min")
                .agg(
                    {
                        "open": "first",
                        "high": "max",
                        ...
                        "close": "last",
                        "volume": "sum",
                        "oi": "last",  # OI is last value in the interval
                    }
                )
                .dropna()
            )
            ...
            if aggregated_df.empty:
                logger.warning(f"No {timeframe_str} data generated after aggregation for {instrument_id}")
            ...
            # Validate aggregated data if enabled
            quality_score = 100.0
            validation_errors = []
            ...
            if self.validation_enabled:
                ...
                quality_score = validation_result.quality_report.quality_score
                validation_errors = validation_result.validation_errors

                if not validation_result.is_valid:
                    logger.warning(
                        f"Aggregated {timeframe_str} data validation failed for instrument {instrument_id}: "
            ...
            ohlcv_records = (
                aggregated_df.reset_index().rename(columns={"date": "ts", "index": "ts"}).to_dict(orient="records")
            )
            ...
            if success:
                logger.info(
                    f"Successfully stored {stored_rows} {timeframe_str} candles "
                    f"for instrument {instrument_id} in {processing_time:.2f}ms"
                )
            ...
            await self.error_handler.handle_error(
                "historical_aggregator",
                f"Error processing {timeframe_str} aggregation for instrument {instrument_id}: {e}",
    ...
    async def _store_with_retry(self, instrument_id: int, timeframe_str: str, ohlcv_records: list[dict]) -> int:
        """
        Store OHLCV records with retry logic.

        Args:
            instrument_id: Instrument identifier
            timeframe_str: Timeframe string (e.g., "5min")
            ohlcv_records: List of OHLCV records to store

        Returns:
            Number of records stored (0 if failed)
        """
        for attempt in range(self.max_retries):
            try:
                await self.ohlcv_repo.insert_ohlcv_data(instrument_id, timeframe_str, ohlcv_records)
                ...
                if attempt == self.max_retries - 1:
                    await self.error_handler.handle_error(
                        "historical_aggregator",
                        f"Failed to store {timeframe_str} data for instrument {instrument_id} "
                        f"after {self.max_retries} attempts: {e}",
                        {
                            "instrument_id": instrument_id,
                            "timeframe": timeframe_str,
                            "records_count": len(ohlcv_records),
                            "attempt": attempt + 1,
                            "error": str(e),
                        },
                    )
                    return 0
                logger.warning(
                    f"Storage attempt {attempt + 1} failed for {timeframe_str} "
                    f"data (instrument {instrument_id}): {e}. Retrying..."
                )
